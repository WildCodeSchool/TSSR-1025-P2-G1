...
# Title
Write-Host "Recherche d'événement par ordinateur :" -ForegroundColor Yellow
Write-Host ""

# Recherche par utilisateur (3ᵉ champ)
$computer = Read-Host "Nom de l'ordinateur"
...
($parts.Count -ge 6) -and ($parts[5] -eq $computer)
}

if ( -not $value )
{
    Write-Host "WARNING ! L'ordinateur n'existe pas sur cette machine." -ForegroundColor Red
}
else
{
    Write-Host "Recherche dans le fichier log_evt.log pour l'ordinateur : $computer" -ForegroundColor Yellow
    Start-Sleep 1
    # Get information and select by regex
    $value = Get-Content -Path $log_file | Where-Object { $_ -match [regex]::Escape($computer) }

    if ( -not $value )
    {
        Write-Host "WARNING ! Aucun événement trouvé pour l'ordinateur : $computer" -ForegroundColor Red
        Write-Host ""
    }
    else
    {
...
# save info 
save_info -label "Recherche d'événement par ordinateur" -value $value
